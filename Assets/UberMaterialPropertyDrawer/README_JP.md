...
## 使い方（ShaderLab）
Shader の `Properties` ブロックで `Uber` 属性を使います。
基本構文は以下です。
```
[Uber(GroupName, DrawerName, optionalArgs...)]
```
`GroupName` はグループ名、`DrawerName` は挙動、`optionalArgs` は追加設定です。
...
- `ToggleUI`: Float/Int プロパティ用のトグル UI。
- `Vector2` / `Vector3`: ラベル配置を調整したベクトル入力。
- `Enum`: Enum のポップアップ。型名指定または `名前, 値` の明示指定に対応。
...
## Tips
- `BeginToggleGroup` と `ToggleUI` は Float/Int プロパティで使用してください。
